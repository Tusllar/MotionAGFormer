...
def gen_video_kpts(video, det_dim=416, num_peroson=1, gen_output=False):
    ...
    # Debug: Log array shapes
    print(f"\n[Debug] Pre-transpose shapes:", flush=True)
    print(f"keypoints.shape: {keypoints.shape}", flush=True)
    print(f"scores.shape: {scores.shape}", flush=True)

    if keypoints.size == 0 or scores.size == 0:
        print("\n[Error] Empty keypoints/scores. Check video or detector.", flush=True)
        return np.zeros((0, 0, 0, 0)), np.zeros((0, 0, 0))

    # Debug: Check dimensions before transpose
    if keypoints.ndim != 4 or scores.ndim != 3:
        print(f"\n[Error] Unexpected dimensions:", flush=True)
        print(f"keypoints.ndim: {keypoints.ndim} (expected 4)", flush=True)
        print(f"scores.ndim: {scores.ndim} (expected 3)", flush=True)
        return keypoints, scores

    # Proceed with transpose
    try:
        keypoints = keypoints.transpose(1, 0, 2, 3)
        scores = scores.transpose(1, 0, 2)
        print("\n[Debug] Transpose successful.", flush=True)
    except ValueError as e:
        print(f"\n[Error] Transpose failed: {e}", flush=True)
        return keypoints, scores
